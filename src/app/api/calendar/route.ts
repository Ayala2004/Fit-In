  ...
  try {
    ...
    if (session.roles.includes("SUPERVISOR")) {
      ...
      if (!userProfile?.supervisorId) {
        return NextResponse.json({ message: "לא נמצא מחוז משויך" }, { status: 400 });
      }
    ...
    const placements = await prisma.placement.findMany({
      ...
      include: {
        ...
        mainTeacher: { select: { firstName: true, lastName: true, id: true, instructorId: true } },
...
export async function DELETE(req: Request) {
  ...
  const isPast = placement && startOfDay(new Date(placement.date)) < startOfDay(new Date());
  
  if (isPast && !session.roles.includes("SUPERVISOR") && !session.roles.includes("INSTRUCTOR")) {
      return NextResponse.json({ message: "רק מפקחת יכולה למחוק דיווחים מהעבר" }, { status: 403 });
  }
