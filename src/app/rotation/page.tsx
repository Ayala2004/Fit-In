...
export default function RotationDashboard() {
  ...
  // 2. טעינת דאשבורד רוטציה בזמן אמת (5 שניות)
  const { 
    data, 
    error, 
    isLoading, 
    mutate 
  } = useSWR("/api/rotation/dashboard", fetcher, {
  ...
  // לוגיקה לחישוב הסטטוס היומי - מחושבת מחדש בכל רענון של SWR
  const status = useMemo(() => {
    ...
    // 1. חיפוש שיבוץ בפועל (היעדרות או החלפה מיוחדת)
    ...
    // מקרה ב': אני רשומה כמחליפה (שיבוץ מיוחד או החלפה פנימית)
  ...
  if (isLoading && !data) return <LoadingScreen message="טוען את סידור העבודה שלך..." />;
  if (error) return <div className="p-10 text-center text-red-500 font-bold">שגיאה בטעינת הלו"ז.</div>;
            ...
            <div className="flex items-center gap-6 text-right w-full md:w-auto">
              ...
              <div>
                <p className="font-black text-[10px] uppercase tracking-[0.15em] opacity-60 mb-1">
                  {status.type === "FIXED" ? "הגן הקבוע שלי להיום:" : status.type === "SPECIAL" ? "שיבוץ מיוחד להיום:" : status.type === "ABSENT" ? "סטטוס נוכחות:" : "היום שלך:"}
                </p>
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    {assignment && <Clock size={16} className="text-slate-300" />}
                  </div>
                  {assignment ? (
                    <div>
                      <p className="font-black text-slate-800">
                        {assignment.manager?.mainManagedInstitutions?.[0]?.name || "גן ללא שם"}
                      </p>
                      <p className="text-xs text-slate-400 font-bold mt-1 flex items-center gap-1">
                        <User size={12} /> גננת אם: {assignment.manager.firstName} {assignment.manager.lastName}
                      </p>
                    </div>
                  ) : (
            ...
            <form onSubmit={handleReportAbsence} className="p-8 space-y-6">
              ...
              <div className="bg-amber-50 p-4 rounded-2xl border border-amber-100 flex gap-3 items-start">
                ...
                <p className="text-xs text-amber-800 font-bold leading-relaxed">
                  שימי לב: המערכת תזהה אוטומטית באיזה גן את אמורה להיות ביום זה ותעדכן את הצוות.
                </p>
              </div>
              <button
                type="submit"
